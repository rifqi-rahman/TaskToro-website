...
const rahmanicLogo = "/logo Rahmanic.png"; // Update this to the correct filename you added
...
export default function Home() {
  ...
  return (
    <div className="bg-background text-foreground min-h-screen flex flex-col font-sans overflow-x-hidden">
      {/* Centered Glassmorphism Navbar with Rahmanic Logo */}
      <nav className="w-full flex justify-center items-center fixed top-6 left-0 z-30 pointer-events-none">
        <div className="flex items-center gap-8 px-10 py-4 bg-white/70 dark:bg-[#18181b]/70 backdrop-blur-xl rounded-full border border-black/10 dark:border-white/10 shadow-card pointer-events-auto max-w-4xl mx-auto">
          {/* Left Links */}
          ...
          <div className="flex items-center justify-center mx-6">
            <Image src={rahmanicLogo} alt="Rahmanic Logo" width={80} height={40} className="rounded-md border border-black/30 dark:border-white/20 bg-white px-2 py-1" />
          </div>
          {/* Right Links */}
          <div className="flex gap-8 text-lg font-medium items-center">
      ...
      <div className="pt-36" /> {/* Spacer for navbar */}

      {/* Hero Section */}
      <section className="w-full flex flex-col items-center text-center py-24 gap-6 px-4">
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          <Image src="/TaskToro icon web.svg" alt="Company Logo" width={100} height={100} className="rounded-2xl shadow-card mb-4" priority />
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.7 }} className="text-6xl font-extrabold tracking-tight text-primary drop-shadow mb-2">
          {companyName}
        </motion.h1>
